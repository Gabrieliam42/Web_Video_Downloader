...
### What each of the scripts does:
  ...
  - Then it finds yt-dlp.exe - Searches bundled library or temporary location file (when packaged) or it searches the current directory
...
`YT-DLP_HQ_Video_w_Subtitle_EN_local.exe` is already bundled with the `yt-dlp.exe` and `_internal` directory included.
#
...
`YT-DLP_HQ_MP3_w_Subtitle_EN_local.exe` is already bundled with the `yt-dlp.exe` and `_internal` directory included.
#
