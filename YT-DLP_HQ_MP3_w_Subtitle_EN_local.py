...
def download_audio(ytdlp_path, video_url, output_widget):
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            print(f"Audio extraction failed with return code: {process.returncode}")
            output_widget.insert(tk.END, f"\nMP3 extraction failed with error code: {process.returncode}\n")
            output_widget.see(tk.END)
            messagebox.showerror("Error", f"MP3 extraction failed with error code: {process.returncode}")
...
def download_and_enable_button(ytdlp_path, video_url, output_widget, download_button):
    try:
        download_audio(ytdlp_path, video_url, output_widget)
    finally:
